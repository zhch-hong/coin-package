  ...
  <div class="user-consume-page" v-loading="loading">
    <el-row style="height: 100%">
      <el-col :span="12">
        ...
        <img src="@/assets/scan-user.png" style="width: 184px; margin: 45px 0 45px 50%; transform: translateX(-50%)" />
        <el-input
          v-model="currentPkg.cardNum"
          placeholder="手动输入卡号"
          style="width: 420px; margin: 10px 5% 10px 50%; transform: translateX(-50%)"
          @input="changeValue"
        >
          <el-button slot="append" @click="getCardInfo">查询</el-button>
        </el-input>
        <div style="margin-left: 50%; transform: translateX(-50%); margin-top: 60px; color: #3d3d3e">
          <p style="font-weight: bold; font-size: 17px">提示：</p>
          <span style="font-weight: bold; font-size: 15px">请读取会员信息或点击会员卡手动输入卡号读取会员信息</span>
        </div>
      </el-col>
      <el-col :span="12" style="background-color: #ffffff; height: 100%">
        <div style="text-align: center; font-size: 24px; color: #2a2a2a; font-weight: bold; padding: 53px 0">当前要解绑的实体会员卡卡号:</div>
        <div style="font-size: 24px; text-align: center; font-weight: bold; color: #2a2a2a; padding: 53px 0">
          {{ currentPkg.cardNum }}
        </div>
        <!--        <h3 v-if="userType">-->
        <!--          卡号信息为:-->
        ...
        <!--        </h3>-->
        <el-divider />
        <div style="text-align: center; font-size: 30px; font-weight: bold; color: #2a2a2a; padding: 106px 0">
          退工本费:
          <span style="color: #f72727">￥{{ currentPkg.backNum }}</span>
        </div>
        <el-button
          type="primary"
          style="width: 170px; margin-left: 50%; transform: translateX(-50%); font-size: 20px; margin-bottom: 70px"
          :disabled="canClose"
        ...
        <el-divider />
        <p style="margin-top: 60px; font-size: 14px; color: #717171; padding: 0 56px">
...
export default {
  ...
  methods: {
    ...
    handlePlaceOrder() {
      this.$confirm('确认解绑后，系统自动增加对应退卡工本费记录。解绑卡内剩余次票、游戏币、积分等自动清空', '解绑实体卡', {
      ...
      this.$api
        ...
        .then((res) => {
          if (res.errCode === 0) {
            ...
            this.$message.success(`查询到卡号信息为${this.cardTypeList.find((item) => item.value === this.userType).label}`)
