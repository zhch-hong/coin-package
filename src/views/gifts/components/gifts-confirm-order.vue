        ...
        <div class="order-detail flex-column" style="flex-basis: 48%; height: 100%">
          ...
          >
            <table class="package-list" style="width: 100%; text-align: center" cellspacing="0" cellpadding="0">
              ...
              <tr v-for="item in prizeList" :key="item.id">
                <td>{{ item.prizeName }}</td>
                <td>{{ item.offValue | MIXIN_Points2Yuan }}</td>
                <td>{{ item.count }}</td>
                <td>{{ $calc.accMul(item.offValue, item.count) | MIXIN_Points2Yuan }}</td>
              </tr>
              <tr>
...
export default {
  ...
  methods: {
    ...
    /**
     * 计算优惠券优惠的价格：
     * 每一张物品的优惠券都要对应哪些商品类型可以使用该优惠券
     * 所以在计算时，要先把这张优惠券支持的商品类型下的商品拿出来参与计算，其他的不参与计算
