  ...
  methods: {
    ...
    printTicket(callback) {
      ...
      LODOP.ADD_PRINT_TEXT(
        ...
        180,
        25,
        `价格：￥${this.MIXIN_Points2Yuan(this.orderInfo.offValueSum)}`
      );
      ...
      this.$api
        ...
        .then((res) => {
          if (Number(this.payNum) === Number(this.orderInfo.coinValueSum)) {
            ...
            // 还需要补现金
            this.leftTotalFee = this.$calc.Subtr(
              this.orderInfo.offValueSum,
              this.MIXIN_Integral2PointsFloor(this.payNum)
            );
